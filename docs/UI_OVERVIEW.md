...
- **Agents** (`/agents`): Übersicht **wer läuft und was eingesetzt wird**: Haupt-Agenten Captain (Operator/Brain/Workflows) und June (OpenClaw/Telegram) mit „Einsatz/Nutzt“-Liste; **Delegationskette** June → ARGUS (Senior Research Engineer) → ATLAS (Sandbox-Validierung), inkl. june-delegate-argus und GATE_ATLAS. **Agent Activity** (`/agents/activity`): Live-Übersicht (Polling ~8s): Drei Spalten June / Argus / Atlas mit „Zuletzt aktiv“, letzte Runs und Status-Balken; unten **Inter-Agent Live** als Timeline (wer wen beauftragt, OVERALL, RECOMMENDATION, ATLAS_*). Einträge aus `logs/agent-activity.jsonl`. June läuft auf dem **Server**; Konfiguration und Exec-Zugriff siehe `docs/TOOL_AGENT_SYSTEM_AUDIT.md` (Abschnitt 3.2). Workflows nach Kategorie mit Tabellen und Badge **„Von UI startbar“** für `ALLOWED_WORKFLOWS`. Details: `docs/TOOL_AGENT_SYSTEM_AUDIT.md`.
