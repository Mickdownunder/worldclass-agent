...
| **Tools** | Alles unter `operator/tools/`: Research-Python-Skripte (`research_*.py`), Shell-Skripte (z.B. `infra-summary.sh`, `run-research-cycle-until-done.sh`). **Tool-Verträge:** `tools/research_tool_registry.py` definiert pro Research-Tool required_env, min_argv, project_id_arg_index; `ensure_tool_context(tool_name)` in den Tools prüft beim Start (bei `RESEARCH_STRICT_TOOL_CONTEXT=1` hart, sonst nur Warnung). Plumber prüft „missing_contracts“. Registry: `knowledge/tools/registry.md`. Factory/Queue/Opportunity entfernt. |
