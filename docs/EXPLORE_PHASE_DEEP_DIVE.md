...
| **research_context_manager.py** | findings/ | `conductor_context.json` (critical + summaries) | **Kontext für Conductor:** Loss-aware tiered compression: Critical (goal-relevant, gaps) preserved; rest condensed. Conductor gets bounded state (~500 tokens target). |
