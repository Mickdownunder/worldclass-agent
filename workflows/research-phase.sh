...
# If flock fails, recover stale lock when previous runner (pid in progress.json) is no longer alive.
...
if ! _acquire_lock; then
  ...
  # Stale lock recovery: pid no longer in /proc, or progress marks runner not alive (crashed/done).
  do_recover=0
...
# Ping June: research run finished so she can read the report and continue (or know Council spawned runs).
SIGNAL_FILE="$OPERATOR_ROOT/knowledge/last_research_complete.json"
mkdir -p "$(dirname "$SIGNAL_FILE")"
python3 - "$PROJECT_ID" "$PHASE" "$SIGNAL_FILE" <<'PING_JUNE' 2>/dev/null || true
import json, sys
from datetime import datetime, timezone
project_id, phase, path = sys.argv[1], sys.argv[2], sys.argv[3]
with open(path, "w") as f:
    json.dump({"project_id": project_id, "phase": phase, "ts": datetime.now(timezone.utc).isoformat()}, f)
PING_JUNE
BRAIN_BIN="$OPERATOR_ROOT/bin/brain"
if [ -x "$BRAIN_BIN" ]; then
  log "Pinging June: triggering brain cycle after research run completed."
  mkdir -p "$OPERATOR_ROOT/logs" 2>/dev/null || true
  nohup env -u HTTP_PROXY -u HTTPS_PROXY -u http_proxy -u https_proxy OPERATOR_ROOT="$OPERATOR_ROOT" "$BRAIN_BIN" cycle --goal "Research run completed for project $PROJECT_ID (phase $PHASE). Read report if done; otherwise advance or wait for Council." >> "$OPERATOR_ROOT/logs/brain_after_research.log" 2>&1 </dev/null &
fi
